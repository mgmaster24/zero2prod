...
impl UserName {
    ...
    /// out validation constraints on subscriber name.
    /// Otherwise, panic!
    pub fn parse(s: String) -> Result<UserName, String> {
        // `.trim()` reutnr a view over the input `s` without
        ...
        // `true` specifies the we want to use the extended grapheme definition
